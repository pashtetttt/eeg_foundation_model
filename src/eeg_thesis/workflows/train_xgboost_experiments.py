...
XGB_CONFIGS: list[dict] = [
    {"name": "xgb_baseline", "n_estimators": 200, "max_depth": 6, "learning_rate": 0.1, "subsample": 0.8, "colsample_bytree": 0.8, "min_child_weight": 1, "reg_lambda": 1.0},
    {"name": "xgb_shallow_lr", "n_estimators": 400, "max_depth": 4, "learning_rate": 0.05, "subsample": 0.9, "colsample_bytree": 0.9, "min_child_weight": 1, "reg_lambda": 1.0},
    {"name": "xgb_deep", "n_estimators": 300, "max_depth": 10, "learning_rate": 0.08, "subsample": 0.8, "colsample_bytree": 0.8, "min_child_weight": 2, "reg_lambda": 1.5},
    {"name": "xgb_regularized", "n_estimators": 250, "max_depth": 6, "learning_rate": 0.1, "subsample": 0.7, "colsample_bytree": 0.7, "min_child_weight": 3, "reg_lambda": 2.0, "reg_alpha": 0.1},
    {"name": "xgb_fast_lr", "n_estimators": 150, "max_depth": 8, "learning_rate": 0.15, "subsample": 0.85, "colsample_bytree": 0.85, "min_child_weight": 1, "reg_lambda": 1.0},
]
...
def build_classifier(cfg: dict) -> XGBClassifier:
    cfg = cfg.copy()
    cfg.pop("name", None)
    return XGBClassifier(random_state=RANDOM_STATE, n_jobs=-1, eval_metric="mlogloss", **cfg)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="XGBoost experiments with balanced sample weights.")
    ...
    parser.add_argument("--features", type=str, default="all", choices=["all", "all_plus_complexity", "alpha", "non_alpha", "selected"], help="all_plus_complexity = legacy features + literature (1 Hz bins, MSE, PSD slope, surrogate).")
    parser.add_argument("--selected-path", type=str, default=str(DEFAULT_SELECTED_FEATURES_PATH))
    parser.add_argument("--literature-surrogate-iters", type=int, default=None, help="Phase-shuffle iterations per channel (default: env EEG_LITERATURE_SURROGATE_ITERS or 25).")
    ...
    log("=" * 72)
    log("Experiments: XGBClassifier (balanced sample_weight per fit)")
    ...
    for cfg_template in XGB_CONFIGS:
    ...
    for cfg_template in XGB_CONFIGS:
        ...
        log(f"    acc={float((y_cv == y).mean()):.4f}  bal_acc={balanced_accuracy_score(y, y_cv):.4f}  macro_f1={f1_score(y, y_cv, average='macro', zero_division=0):.4f}")
    ...
    out = args.results_dir / f"results_xgb_experiments_{args.eyes}_{args.features}_{ts}.txt"
