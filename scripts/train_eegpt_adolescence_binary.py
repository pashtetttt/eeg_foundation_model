#!/usr/bin/env python3
"""Train EEGPT for binary adolescence-vs-rest with validation, early stopping, and run artifacts."""
    ...
    *,
    ...
    with torch.no_grad():
        for xb, yb in _iterate_minibatches(X, y, batch_size, shuffle=False):
            ...
            loss = criterion(logits, yt)
            losses.append(float(loss.item()))
            logits_chunks.append(logits.squeeze(-1).cpu().numpy())
    raw = np.concatenate(logits_chunks, axis=0) if logits_chunks else np.array([], dtype=float)
    probs = 1.0 / (1.0 + np.exp(-raw)) if raw.size else raw
    y_pred = (probs >= threshold).astype(np.int64) if probs.size else np.array([], dtype=np.int64)
    ...
    return vloss, bal, mf1, y_pred, probs
...
def parse_args() -> argparse.Namespace:
    ...
    ap = argparse.ArgumentParser(description="EEGPT binary training (adolescence vs rest) with logging.")
    ...
    ap.add_argument("--early-stopping-patience", type=int, default=int(train_cfg.get("early_stopping_patience", 8)))
    ap.add_argument(
...
def main() -> None:
    ...
    tag = args.run_name or (Path(args.config).stem if args.config else "adolescence_binary")
    out_dir = args.output_dir
    if out_dir is None:
        out_dir = ROOT / "results" / "eegpt_runs" / f"{ts}_adolescence_binary_{tag}"
    else:
    ...
    y_bin, class_names = to_binary_adolescence(ds.y, ds.class_names)

    sss = StratifiedShuffleSplit(n_splits=1, test_size=TEST_SIZE, random_state=RANDOM_STATE)
    train_val_idx, test_idx = next(sss.split(np.arange(len(y_bin)), y_bin))
    X_trv, X_test = ds.X[train_val_idx], ds.X[test_idx]
    y_trv, y_test = y_bin[train_val_idx], y_bin[test_idx]
    ...
    _, _, _, y_pred, probs = _eval_binary(
        model,
        X_test,
        y_test,
        batch_size=args.batch_size,
    ...
    f1_pos = float(f1_score(y_test, y_pred, pos_label=1, zero_division=0)) if len(y_test) else 0.0
    report = classification_report(y_test, y_pred, target_names=class_names, zero_division=0)
    cm = confusion_matrix(y_test, y_pred, labels=[0, 1]).tolist()
    try:
        auc = float(roc_auc_score(y_test, probs)) if len(np.unique(y_test)) > 1 and probs.size else None
    except ValueError:
        auc = None
    ...
    plot_confusion_matrix(cm, list(class_names), out_dir / "confusion_matrix_test.png", title="Test confusion matrix")
